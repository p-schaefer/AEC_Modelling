...
# Finalize no human impact data -------------------------------------------

ref_pred_data<-int_data_out %>% 
  select(# Taxa Info
    gen_Region=region,
    gen_ProvReachID=ProvReachID,
    gen_link_id=link_id,
  ...
  rename_with(.cols = starts_with("LDI_Natural"),~gsub("_Natural","",.x)) %>% 
  #mutate(across(starts_with("LDI_"),~0)) %>% 
  select(-contains(c("Urban","Agriculture","Agricultural","NonNatural","Exposed"))) %>% 
  rowwise() %>% 
  mutate(across(starts_with("br_") & ends_with("_HAiFLO_prop"),~.x/sum(c_across(starts_with("br_") & ends_with("_HAiFLO_prop"))))) %>% 
  ...
  mutate(across(starts_with("nr_") & ends_with("_iFLS_prop"),~.x/sum(c_across(starts_with("nr_") & ends_with("_iFLS_prop"))))) %>% 
  mutate(across(starts_with("nr_") & ends_with("_lumped_prop"),~.x/sum(c_across(starts_with("nr_") & ends_with("_lumped_prop"))))) %>% 
  ungroup() %>% 
  mutate(case_weight=importance_weights(1)) %>% 
  select(gen_Region,any_of(colnames(model_data0))) 
...
for (i in setdiff(colnames(pred_data_fin),colnames(ref_pred_data))){
  ref_pred_data[[i]]<-0
}

write_rds(ref_pred_data,file.path("data","final","Prediction_ref_finaltaxa_data.rds"))
