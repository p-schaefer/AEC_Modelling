calc_fish_ep<-function(df) {
  ...
  ep_tbl<-df %>% 
    ...
    summarise(
      ...
      # Tolerance
      Tol_tol_Bioperc=sum(EstimatedBiomass_perc[grepl("tolerant",Tolerance)],na.rm=T),
      Tol_intol_Bioperc=sum(EstimatedBiomass_perc[grepl("intolerant",Tolerance)],na.rm=T),
      Tol_tol_Abuperc=sum(NumberOfFish_perc[grepl("tolerant",Tolerance)],na.rm=T),
      Tol_intol_Abuperc=sum(NumberOfFish_perc[grepl("intolerant",Tolerance)],na.rm=T),
      Tol_tol_Bio=sum(EstimatedBiomass_perc[grepl("tolerant",Tolerance)],na.rm=T),
      Tol_intol_Bio=sum(EstimatedBiomass_perc[grepl("intolerant",Tolerance)],na.rm=T),
      Tol_tol_Abu=sum(NumberOfFish_perc[grepl("tolerant",Tolerance)],na.rm=T),
      Tol_intol_Abu=sum(NumberOfFish_perc[grepl("intolerant",Tolerance)],na.rm=T),
      Tol_tol_Rich=length(NumberOfFish_perc[grepl("tolerant",Tolerance)]),
      Tol_intol_Rich=length(NumberOfFish_perc[grepl("intolerant",Tolerance)]),
      # Individual Species - 
