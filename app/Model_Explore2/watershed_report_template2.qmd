...
sel_strms <- sf::read_sf(fp,"AEC_Streams") %>% 
  sf::st_transform(4326) %>% 
  filter(grepl("w01|w02|w03|w04|w05|w06|w12|w13|w14",AEC_Region_sub))
...
for (i in names(col_brks)[c(9,17,14)]){
  lyr <- sel_strms %>% 
    select(ProvReachID) %>% 
    left_join(
      sel_modelpredictions %>% 
...
out_map <- out_map %>% 
  leaflet::addLayersControl(
    ...
    overlayGroups = ep_rn(names(col_brks)[c(9,17,14)]),
    position = "topleft",
    options = leaflet::layersControlOptions(collapsed = F)
  ) %>% 
    #leaflet::hideGroup(ep_rn(c(CalcEP,taxa))[-c(1)])
  leaflet::hideGroup(ep_rn(names(col_brks)[c(9,17,14)])[-c(1)])
